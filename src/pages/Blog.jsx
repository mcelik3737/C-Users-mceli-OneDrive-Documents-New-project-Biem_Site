...
import { localMediaUrl } from "@/lib/assets";
  ...
  return (
    <>
      ...
      <PageHero
        ...
        subtitle="Kritik haberleşme, RF kapsama, raylı sistem, tünel ve maden projelerine dair teknik notlar, proje hikayeleri ve sektör gelişmeleri."
        buttons={[{ label: "Teklif Alın", to: "/iletisim" }]}
      />

      <section className="py-16 lg:py-24">
        <div className="max-w-7xl mx-auto px-4 lg:px-6">
          <div className="flex flex-wrap gap-2 mb-10" aria-label="Blog kategorileri">
            {CATEGORIES.map((cat) => (
              <button
                key={cat.value}
                type="button"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((post) => (
              <Link
                key={post.id}
                to={`/blog/${post.id}`}
                className="group rounded-xl border border-border/50 bg-card/50 hover:border-primary/30 hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col"
              >
                {post.cover_image ? (
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <h2 className="text-base font-semibold text-foreground mb-2 leading-snug group-hover:text-primary transition-colors line-clamp-2">
                    {post.title}
                  </h2>
