...
# Kullanim: chmod +x indir-mac.sh && ./indir-mac.sh

BASE="https://media.base44.com/"
HEDEF="$(cd "$(dirname "$0")" && pwd)/biem-dosyalar"
mkdir -p "$HEDEF/images" "$HEDEF/files"

read -r -d '' LISTE << 'LISTEEOF'
...
TOPLAM=$(echo "$LISTE" | wc -l | tr -d " ")
I=0; OK=0; HATA=0
while IFS= read -r YOL; do
  I=$((I+1))
  AD=$(basename "$YOL")
  case "$YOL" in files/*) KLASOR="files";; *) KLASOR="images";; esac
  CIKTI="$HEDEF/$KLASOR/$AD"
  if [ -f "$CIKTI" ]; then echo "[$I/$TOPLAM] atlandi: $AD"; OK=$((OK+1)); continue; fi
  if curl -fsSL --max-time 60 "$BASE$YOL" -o "$CIKTI"; then
    echo "[$I/$TOPLAM] OK  $AD"; OK=$((OK+1))
  else
    echo "[$I/$TOPLAM] HATA $AD"; echo "$YOL" >> "$HEDEF/hatalar.txt"; HATA=$((HATA+1))
  fi
done <<< "$LISTE"

echo ""
echo "================================"
echo "Basarili: $OK   Hatali: $HATA"
echo "Klasor: $HEDEF"
echo "================================"